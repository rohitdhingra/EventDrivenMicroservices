...
public class KafkaAdminClient {
	...
	private void checkMaxRetry(int retry, Integer maxRetry) {
		if(retry > maxRetry)
		{
			throw new KafkaClientException("Reached max number of retry for getting Topics!!");
		}
	}
	...
	private Collection<TopicListing> getTopics()
	{
		...
		try {
			...
			throw new KafkaClientException("Reached max number of retry for getting Topics!!",e);
		}
		return topics;
	}
	...
	{
		try {
			return webClient.method(HttpMethod.GET).uri(kafkaConfigData.getSchemaRegistryUrl()).exchange().map(ClientResponse::statusCode).block();
		} catch (Exception e) {
